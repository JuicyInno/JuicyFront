...
describe('Test <StatusWithText/> component', () => {
    ...
    it('should be render StatusWithText green', () => {
        ...
        expect(container.getElementsByClassName('rf-status-with-text__success')).toHaveLength(1);
        expect(container.getElementsByClassName('rf-status-with-text__default')).toHaveLength(0);
    })

    it('should be render StatusWithText blue', () => {
        const { container } = render(<StatusWithText statusText='Согласовано' criticality='0' />);
        expect(container.getElementsByClassName('rf-status-with-text__default')).toHaveLength(1);
    })

    it('should be render StatusWithText yellow', () => {
        const { container } = render(<StatusWithText statusText='На доработку' criticality='2' />);
        expect(container.getElementsByClassName('rf-status-with-text__warning')).toHaveLength(1);
        expect(container.getElementsByClassName('rf-status-with-text__success')).toHaveLength(0);
    })

    it('should be render StatusWithText red', () => {
        const { container } = render(<StatusWithText statusText='Отклонено' criticality='1' />);
        expect(container.getElementsByClassName('rf-status-with-text__error')).toHaveLength(1);
        expect(container.getElementsByClassName('rf-status-with-text__warning')).toHaveLength(0);
    })
})